...
struct entity
{
	...
	void onFree()
	{
		...
		if (aabb)				 aabb.free();
		if (damage_taken_sprite) damage_taken_sprite.free();
	}
};
...
public:
	...
	Factory(int a_size) : m_size(a_size), entities(a_size), transforms(a_size), rigidbodies(a_size), controllers(a_size),
				sprites(a_size), damage_taken_sprites(a_size), AABBs(a_size)
	{}
	...
	ObjectPool<entity>::iterator spawnPlayer(unsigned sprite_id, unsigned damage_taken_sprite_id)
	{
		...
		retVal->trans->m_position = Vec2{ 400, 100 };
	...
	ObjectPool<entity>::iterator spawnCreep(unsigned sprite_id, unsigned damage_taken_sprite_id, Vec2 a_pos)
	{
		auto retVal = entities.push();
		if (!retVal) return retVal;
		retVal->factory = this;
		...
		retVal->trans->m_position = a_pos;
		retVal->trans->m_scale = Vec2{ 100, 100 };

		retVal->aabb->m_pos = retVal->trans->m_position;
		retVal->aabb->m_he = Vec2(50, 50);

		retVal->rb->mass = 1;
		retVal->rb->setVelocity(Vec2(0, 0));
		...
		*retVal->cntlr = new ControllerCreep();
	...
	ObjectPool<entity>::iterator spawnBoss(unsigned sprite_id, unsigned damage_taken_sprite_id)
	{
		...
		retVal->trans->m_position = Vec2{ 900, 400 };
